...
// Add a global declaration for the API key on the window object
declare global {
  interface Window {
...
// Check if the config.js file is loaded and the key is set.
// This is a user-friendly way to guide them if they forget to add their key.
if (!window.GEMINI_API_KEY || window.GEMINI_API_KEY === "YOUR_GEMINI_API_KEY_GOES_HERE") {
  alert("API Key not found. Please create a 'config.js' file, add your Google Gemini API key, and refresh the page.");
  throw new Error("API_KEY is not set in config.js.");
}
